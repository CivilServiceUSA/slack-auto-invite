...
(function() {
  ...
  (function() {
    ...
    $form.addEventListener('submit', function(event) {
      ...
      var validEmail = regExpEmail.test($email.value);

      var regExpInvite = new RegExp("^[a-zA-Z0-9]{6}$");
      var validInvite = regExpInvite.test($invite.value);
      ...
      if (validEmail && validInvite) {
        $.ajax({
          ...
          url: 'slack.php',
          dataType: 'json',
          data: { email: $email.value, invite: $invite.value },
          success: function (response) {
            if(response.success) {
              ...
              $submit.disabled = false;
              $message._show('error', response.error);
            }
          }
        });
      } else if (!validEmail) {
